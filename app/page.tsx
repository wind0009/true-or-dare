...
export default function Game() {
  ...
  return <main className="app"><section className="game-shell">
    ...
    <section className={`game-area phase-${phase}`}>
      ...
      {(phase === "wheel" || phase === "spinning") && <div className="stage spin-stage"><p className="eyebrow">{phase === "wheel" ? "À TOI DE JOUER" : "LA BOUTEILLE TOURNE"}</p><div className="table-scene"><div className="pointer">▼</div><div className="wheel"><div className="wheel-disk" style={{ background: wheelGradient }}>{players.map((item, itemIndex) => <span key={item.name} className="wheel-name" style={{ transform: `rotate(${itemIndex * (360 / players.length)}deg) translateY(-104px) rotate(${-itemIndex * (360 / players.length)}deg)` }}>{item.name}</span>)}</div><div className="wheel-hub"><button className={phase === "spinning" ? "bottle-button is-spinning" : "bottle-button"} onClick={spinBottle} disabled={phase === "spinning"} aria-label="Faire tourner la bouteille" style={{ transform: `rotate(${bottleRotation}deg)` }}><img src="/assets/spin-bottle.png" alt="Bouteille de jeu" /></button></div></div></div><h1>{phase === "wheel" ? "Fais tourner la bouteille" : "La bouteille choisit…"}</h1><p className="stage-copy">{phase === "wheel" ? "Clique sur la bouteille : elle choisira le prochain joueur." : "Elle ralentit… qui sera désigné ?"}</p></div>}
